import { findBattleByIdWithPopulated, findOneSubmission, updateSubmissionById } from '../../repositories/index.js';
import { triggerAiAnalysis, triggerAiBattleReview } from '../../ai/services/aiAnalysis.service.js';

export const getAiReviewService = async (battleId, userId) => {
  const battle = await findBattleByIdWithPopulated(battleId, ['players.user']);
  if (!battle) {
  ...
  if (!isParticipant) {
    const err = new Error('Only participants can view AI review');
    err.status = 403;
    throw err;
  }

  if (battle.status !== 'ended') {
    const err = new Error('AI review is only available after battle ends');
    err.status = 400;
    throw err;
  }
  ...
  const p1Sub = await findOneSubmission({ userId: p1.user._id, problemId: battle.problem, battleId: battle._id }, { createdAt: -1 });
  const p2Sub = await findOneSubmission({ userId: p2.user._id, problemId: battle.problem, battleId: battle._id }, { createdAt: -1 });
  ...
  const checkAndTrigger = async (sub) => {
    ...
    if (isOldFormat || ['none', 'failed', 'expired'].includes(sub.aiAnalysisStatus) || (!sub.aiAnalysis && sub.aiAnalysisStatus !== 'pending')) {
      ...
      await updateSubmissionById(sub._id, { 
        aiAnalysisStatus: 'pending',
        ...(isOldFormat && { aiAnalysis: null })
      });
